...
public class RNHTMLtoPDFModule extends ReactContextBaseJavaModule {
    ...
    public void convert(final ReadableMap options, final Promise promise) {
        try {
            ...
            convertToPDF(htmlString, destinationFile, options, Arguments.createMap(), promise);
        } catch (Exception e) {
            promise.reject(e.getMessage());
        }
    }

    private String convertToPDF(String htmlString, File file, ReadableMap options, WritableMap resultMap,
            Promise promise) throws Exception {
        try {
            PdfConverter pdfConverter = PdfConverter.getInstance();
            pdfConverter.convert(mReactContext, htmlString, file, options, resultMap, promise);
